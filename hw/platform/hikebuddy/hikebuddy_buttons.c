/*
 * hikebuddy_buttons.c
 * Implementation of generic hikebbuddy button HAL.
 * RebbleOS
 *
 ...
 *
 * The hikebuddy_buttons implementation comes in two major parts: the HAL
 ...
 * are two header files involved: hikebuddy_buttons.h, which provides the OS HAL
 * (and is expected to be included by a platform's platform.h), and
 * hikebuddy_buttons_platform.h, which is intended to be included by a file in
...
/* On esp32,the buttons are mapped to the io expander */
void hw_button_init(void)
{
    for (int i = 0; i < HW_BUTTON_MAX; i++) {
        const hikebuddy_button_t *btn = &platform_buttons[i];
        //
        // /* Initialize the GPIO. */
        // GPIO_InitTypeDef gpioinit;
        esp32_power_request(ESP32_POWER_AHB1, btn->gpio_clock);
        //
        ...
        //
        esp32_power_release(ESP32_POWER_AHB1, btn->gpio_clock);
        //
        // /* Set up the pin external interrupt. */
        // EXTI_InitTypeDef extiinit;
        //
        esp32_power_request(ESP32_POWER_APB2, RCC_APB2Periph_SYSCFG);
        //
        ...
        esp32_power_release(ESP32_POWER_APB2, RCC_APB2Periph_SYSCFG);
    }
}
 ...
 * making the macro figure out which hikebuddy_button_t we really mean..
 */
void hikebuddy_buttons_raw_isr()
{
    for (int i = 0; i < HW_BUTTON_MAX; i++) {
        const hikebuddy_button_t *btn = &platform_buttons[i];
...
int hw_button_pressed(hw_button_t button_id)
{
    ...
    const hikebuddy_button_t *btn = &platform_buttons[button_id];
